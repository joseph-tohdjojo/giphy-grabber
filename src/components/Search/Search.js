...
class Search extends Component {
  ...
  render() {
    const searchHistory = this.props.searchHistory.map((search, i) => <li key={i}><a href="#" onClick={this.handlePrevSearchClick(search)}>{search}</a></li>)
    return (
      <form onSubmit={this.handleSubmit} className={styles.wrapper}>
        ...
        <button
          ...
          type="submit"
        >
          <img src="https://giphy.com/static/img/animations/search_animation_active.gif" alt="Submit search" />
        </button>

        <p className={`${styles.prevSearchesLabel} ${searchHistory.length > 0 ? '' : styles.hide}`}>Previous searches:</p>
